...
```bash
npm install          # 의존성 설치
npm run dev          # 개발 서버 시작 (Turbopack, localhost:3100)
...
│   └── features/     # 기능별 복합 컴포넌트 (LogTable, AlertForm 등)
├── lib/              # 공유 유틸리티, 상수, 헬퍼
├── hooks/            # 커스텀 React 훅
...
- **컴포넌트**: named export 사용 (`export function Button`), default export 사용하지 않음
